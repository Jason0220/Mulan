...
if (( $# < 4 )) ; then
	...
	echo "BES_DIR: The parent dir of repository "Mulan-BES2500L""
	echo "NXP_VER: The NXP595 firmware version to be included in the OTA package"
	echo "BES_VER: The BES2500 firmware version to be included in the OTA package"
	logger "params error"
...
function ota_generation() {
	cd $OTA_PATH
	echo -e "\ncd $(pwd)" | tee -a $LOG_FILE
	python ota_packager.py 2>&1 | tee -a $LOG_FILE
	echo "Generate OTA upgrade bin file result ${PIPESTATUS[0]}" | tee -a $LOG_FILE
	for file in upgrade_2024-*.bin; do
    	    mv "$file" "upgrade.bin"
	done
	echo "Rename OTA upgrade file as upgrade.bin result ${PIPESTATUS[0]}" | tee -a $LOG_FILE
